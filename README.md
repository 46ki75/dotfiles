...
### 7. Global mise toolchain

`mise/.config/mise/config.toml` defines pinned global tool versions and is linked to `~/.config/mise/config.toml`. Project mise configs override the global defaults for tools they declare.

With mise installed and activated in your shell, deploy and install the global tools:
...
If `~/.config/mise/config.toml` already exists as a regular file, reconcile it with the repository config before stowing. Edit the tracked config or use `mise use --global --pin <tool>@<version>` to update a tool, then review the Git diff.
